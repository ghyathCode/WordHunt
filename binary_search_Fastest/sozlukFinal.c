...
//                         
  ...
  #pragma omp parallel for default(none) private(i,j,coor,str_coor_stack) shared(game_grid,thread_count, pdictionary, coor_def,str_coor_stack_def) schedule(static,1)
   for (i=0;i<4;i++){
       int top = -1;
       thread_count = omp_get_num_threads();     
       int my_rank = omp_get_thread_num();
